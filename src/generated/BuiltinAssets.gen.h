...
inline constexpr std::string_view kStage5Runtime = R"DOA((function(){
    ...
    window.__DearOreUI__.ipc = {
        ...
        callHost: function(method, args) {
            return new Promise(function(resolve, reject) {
                try {
                    if (window.__DearOreUI__.dispatchUsed) throw new Error('ViewDispatchAlreadyUsed');
                    if (!hasFacet) throw new Error('HostBridgeUnavailable');
                    window.__DearOreUI__.dispatchUsed = true;
                    ...
                    var timer = setTimeout(function() { window.__DearOreUI__.bus.off(id); reject(new Error('HostCallTimeout')); }, 5000);
                    window.__DearOreUI__.bus.on(id, function(resp) { clearTimeout(timer); resolve(resp); });
                    if (!dearOreUiFacetTrigger(JSON.stringify(request))) {
                        clearTimeout(timer);
                        window.__DearOreUI__.bus.off(id);
                        throw new Error('FacetUnavailable');
                    }
                } catch (e) { reject(e); }
            });
        },
...
inline constexpr std::string_view kStage7UiBootstrap = R"DOA((function(){
    window.__DearOreUI__ = window.__DearOreUI__ || {};
